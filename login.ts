class Login_Page {  
    login() {  
     var a = 0;  
     var UserName = ( < HTMLTextAreaElement > (document.getElementById('User Name'))).value;  
     var txtpwd = ( < HTMLTextAreaElement > (document.getElementById('Pwd'))).value;  
     if ('User Name'.length != 0 && txtpwd.length != 0) {  
      ...
      rs.Open("select * from Login_Info where Emp_Name='" +'User Name'+ "' and Passwrd='" + txtpwd + "'", connection);  
      while (!rs.eof) {  
